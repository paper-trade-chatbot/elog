...
func getLogLevelString(level int) string {
	if level == LOG_LEVEL_DEBUG {
		return "\x1b[0;30;42mDEBUG\x1b[0m"
	} else if level == LOG_LEVEL_INFO {
		return "\x1b[0;30;47m INFO\x1b[0m"
	} else if level == LOG_LEVEL_WARN {
		return "\x1b[0;30;43m WARN\x1b[0m"
	} else if level == LOG_LEVEL_ERROR {
		return "\x1b[0;30;41mERROR\x1b[0m"
	} else if level == LOG_LEVEL_FATAL {
		return "\x1b[0;37;41mFATAL\x1b[0m"
...
func (el *EasyLogger) getHeader(level int) string {
	...
	if el.mode == WITH_FILE_LINE {
		...
		return fmt.Sprintf("%s \x1b[37;100m%s\x1b[0;0m [file:%s line:%d]", getLogLevelString(level), getTimeNowStr(), file, line)

	} else {
		return fmt.Sprintf("%s \x1b[37;100m%s\x1b[0;0m", getLogLevelString(level), getTimeNowStr())
	}
